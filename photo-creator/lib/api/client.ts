...
async function request<T>(
  ...
  const fetchOptions: RequestInit = {
    ...
    // 默认 15s 超时，避免构建期/运行期请求无限挂起（调用方可通过 options.signal 覆盖）
...
export const blogger = {
  get: (options?: RequestInit) =>
    get<Blogger>("/api/v1/public/blogger", undefined, options),
};

export const articles = {
  list: (
    params?: PaginationParams & { category_id?: string; keyword?: string },
    options?: RequestInit
  ) => get<PaginatedResponse<Article>>("/api/v1/public/articles", params, options),
  hot: (count?: number) =>
  ...
  detail: (slug: string, options?: RequestInit) =>
    get<ArticleDetail>(`/api/v1/public/articles/${slug}`, undefined, options),
  view: (slug: string) =>
...
export const portfolios = {
  list: (
    params?: PaginationParams & { keyword?: string; category_id?: string },
    options?: RequestInit
  ) => get<PaginatedResponse<Portfolio>>("/api/v1/public/portfolios", params, options),
  detail: (id: string, options?: RequestInit) =>
    get<PortfolioDetail>(`/api/v1/public/portfolios/${id}`, undefined, options),
};
