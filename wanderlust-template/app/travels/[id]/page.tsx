      ...
      {[1, 2, 3, 4, 5].map((star) => (
        <Star
          ...
          className={`h-3.5 w-3.5 ${
            star <= Math.round(clamped) ? "fill-flame text-flame" : "text-text-subtle"
          }`}
          strokeWidth={0}
        />
      ))}
  ...
  return (
    <div className="flex flex-1 flex-col bg-background">
      ...
      <header className="mx-auto w-full max-w-5xl px-4 py-6 sm:px-6 lg:px-8">
        <div className="relative aspect-[21/9] overflow-hidden rounded-radius-lg shadow-card">
          ...
          <div className="absolute inset-x-0 bottom-0 p-6 md:p-8">
            <p className="mb-2 flex items-center gap-1.5 text-xs font-medium uppercase tracking-[0.2em] text-white/90">
              <MapPin className="h-3.5 w-3.5" strokeWidth={1.8} />
              {travel.destination}
              {travel.region ? ` · ${travel.region}` : ""}
            </p>
            <h1 className="max-w-3xl font-display text-2xl font-normal leading-tight text-white md:text-4xl">
              {travel.title}
            </h1>
            <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-white/85">
              ...
              {travel.rating > 0 && (
                <span className="flex items-center gap-1.5">
                  <Star className="h-4 w-4 fill-flame text-flame" strokeWidth={0} />
      ...
      <div className="mx-auto w-full max-w-3xl flex-1 px-4 py-8 sm:px-6 lg:px-8 md:py-10">
        ...
        {/* Summary */}
        {travel.summary && (
          <blockquote className="mt-8 rounded-radius-md border-l-[3px] border-accent bg-accent-subtle/50 py-3 pl-4 pr-3 text-base italic leading-relaxed text-text-secondary md:pl-5">
            {travel.summary}
          </blockquote>
        )}
        ...
        {attractions.length > 0 && (
          <section className="mt-14">
            <h2 className="mb-6 font-display text-2xl text-text-primary">
              景点推荐
              <span className="ml-3 font-hand text-xl font-medium text-accent/70">
                Spots
              </span>
            </h2>
            <div className="space-y-6">
              {attractions.map((attraction, i) => {
                ...
                return (
                  <div
                    key={`attraction-${i}`}
                    className="flex flex-col gap-4 rounded-radius-md border border-border bg-surface p-4 shadow-card sm:flex-row md:p-5"
                  >
                    {image ? (
                      <div className="relative aspect-[4/3] w-full shrink-0 overflow-hidden rounded-radius-sm sm:w-44">
                        <Image
                          src={image}
                          alt={name || `景点 ${i + 1}`}
                          fill
            ...
            <div className="space-y-4">
              {reviews.map((review, i) => {
                ...
                return (
                  <div
                    key={`review-${i}`}
                    className="rounded-radius-md border border-border bg-surface p-5 shadow-card"
                  >
