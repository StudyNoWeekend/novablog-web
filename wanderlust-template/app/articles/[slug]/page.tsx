...
export default async function ArticleDetailPage({ params }: PageProps) {
  ...
  if (!config.article_enabled) {
    return <ModuleDisabled moduleLabel="游记" />;
  }
    ...
    <div className="flex flex-1 flex-col bg-background">
      ...
      {article.cover_image ? (
          ...
          <div className="relative aspect-[21/9] overflow-hidden rounded-radius-lg shadow-card">
            ...
            <div className="absolute inset-x-0 bottom-0 p-6 md:p-8">
              {article.category_name && (
                <span className="mb-3 inline-block rounded-full bg-white/90 px-3 py-1 text-xs font-medium text-text-secondary backdrop-blur-sm">
                  {article.category_name}
                </span>
              )}
        ...
        <header className="mx-auto w-full max-w-3xl px-4 pt-4 sm:px-6 lg:px-8">
          {article.category_name && (
            <span className="mb-3 inline-block rounded-full bg-accent-subtle px-3 py-1 text-xs font-medium text-accent">
              {article.category_name}
            </span>
          )}
          <h1 className="font-display text-3xl font-normal leading-tight text-text-primary md:text-4xl">
            {article.title}
          </h1>
        </header>
      )}
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            {article.tag_names.map((tag) => (
              <Link
                ...
                className="min-h-8 cursor-pointer rounded-full border border-accent/25 bg-accent-subtle px-3 py-0.5 text-xs text-accent transition-colors duration-200 hover:border-accent hover:bg-accent hover:text-white"
              >
                # {tag}
              </Link>
            ))}
