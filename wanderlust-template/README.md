...
| `/music` | 旅途音乐：歌曲列表（右下角悬浮播放器，B 站外链 iframe 播放）+ 第三方歌单 |
| `/about` | 关于我（博主资料 + 标签 + 社交链接） |

导航与页面展示由后端「模块开关配置」（`/api/v1/public/module-config`）驱动：关闭文章模块会隐藏游记入口，关闭旅行模块会隐藏目的地/攻略入口，关闭器材模块会隐藏装备入口，关闭音乐模块会隐藏音乐入口。
