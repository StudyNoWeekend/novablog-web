...
export function ArticlesListContent() {
  ...
  return (
    <>
      {/* Category Filter */}
      <section className="sticky top-20 z-30 border-b border-border bg-background/95 py-4 backdrop-blur-md">
        <div className="no-scrollbar mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          {loading ? (
                    ...
                    <div className="flex flex-wrap gap-2">
                      {tags.map((tag) => (
                        <Link
                          ...
                          className="min-h-8 cursor-pointer rounded-full border border-border bg-background-soft px-3 py-1 text-xs text-text-muted transition-colors duration-200 hover:border-accent hover:text-accent-hover"
                        >
                          #{tag.name}
                        </Link>
                      ))}
            ...
            <div className="flex flex-col items-center justify-center py-24 text-center">
              <p className="font-hand text-3xl text-text-subtle">empty plate…</p>
              <p className="mt-2 text-lg text-text-muted">
                {params.keyword
                  ? `未找到与「${params.keyword}」相关的美食日记`
                  : "该分类下暂无美食日记"}
              </p>
              <Link
                href="/articles"
