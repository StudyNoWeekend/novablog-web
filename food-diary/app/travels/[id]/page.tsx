...
export async function generateMetadata({ params }: PageProps) {
  ...
  if (id === FALLBACK_ID) {
    return { title: "旅行美食未找到" };
  }
  const travel = await getTravelById(id);
  if (!travel) return { title: "旅行美食未找到" };
...
export default async function TravelDetailPage({ params }: PageProps) {
  ...
  if (!config.travel_enabled) {
    return <ModuleDisabled moduleLabel="旅行美食" />;
  }
  ...
  return (
    <div className="flex flex-1 flex-col bg-background">
      ...
      <header className="mx-auto w-full max-w-5xl px-4 py-6 sm:px-6 lg:px-8">
        <div className="relative aspect-[21/9] overflow-hidden rounded-lg shadow-card">
          {travel.cover_image ? (
            <Image
          ...
          <div className="absolute inset-x-0 bottom-0 p-6 md:p-8">
            ...
            <h1 className="max-w-3xl font-display text-2xl font-normal leading-tight text-white drop-shadow md:text-4xl">
              {travel.title}
            </h1>
      ...
      <div className="mx-auto w-full max-w-3xl flex-1 px-4 py-8 sm:px-6 lg:px-8 md:py-10">
        ...
        {/* Summary */}
        {travel.summary && (
          <blockquote className="mt-8 rounded-r-lg border-l-[3px] border-accent bg-surface-highlight py-3 pl-4 pr-4 text-base italic leading-relaxed text-text-muted md:pl-5">
            {travel.summary}
          </blockquote>
        )}
            ...
            <div className="space-y-6">
              {attractions.map((attraction, i) => {
                ...
                return (
                  <div
                    key={`attraction-${i}`}
                    className="flex flex-col gap-4 rounded-lg border border-border bg-surface p-4 shadow-card sm:flex-row md:p-5"
                  >
                    {image ? (
                      <div className="relative aspect-[4/3] w-full shrink-0 overflow-hidden rounded-md sm:w-44">
                        <Image
                          src={image}
                          alt={name || `推荐 ${i + 1}`}
                          fill
                    ...
                    <div className="min-w-0 flex-1">
                      {name && (
                        <h3 className="flex items-center gap-2 text-lg font-semibold text-text-primary">
                          <Heart className="h-4 w-4 shrink-0 fill-berry text-berry" strokeWidth={0} />
                          {name}
                        </h3>
                      )}
                      {description && (
        ...
        {itinerary.length > 0 && (
          <section className="mt-14">
            ...
            <ol className="relative space-y-8 border-l-2 border-accent/25 pl-6">
              {itinerary.map((item, i) => {
                ...
                return (
                  <li key={`itinerary-${i}`} className="relative">
                    ...
                    {label && (
                      <h3 className="font-display text-base text-accent-hover">
                        {label}
                      </h3>
                    )}
                    {description && (
