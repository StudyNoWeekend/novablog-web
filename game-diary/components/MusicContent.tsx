...
function PlatformBadge({ platform }: { platform: string }) {
  return (
    <span className="shrink-0 rounded-full bg-accent-subtle px-2.5 py-0.5 text-[11px] font-semibold text-accent-hover">
      {platform}
    </span>
  );
}
...
function PlaylistCard({ playlist }: { playlist: Playlist }) {
  return (
    <a
      ...
      className="group block rounded-lg border border-border bg-surface p-4 shadow-card transition-all duration-200 ease-out hover:-translate-y-1 hover:border-accent/40 hover:shadow-card-hover"
    >
      {/* Cover */}
      <div className="relative aspect-[3/2] w-full overflow-hidden rounded-md bg-surface-elevated">
        {playlist.cover_url ? (
          <Image
        ...
        <span className="absolute right-2 top-2 flex h-7 w-7 items-center justify-center rounded-full bg-black/60 opacity-0 transition-opacity duration-200 group-hover:opacity-100">
      ...
      <div className="mt-4 space-y-2">
        <div className="flex items-start justify-between gap-2">
          <h3 className="line-clamp-1 font-heading text-sm font-bold text-text-primary">
            {playlist.title}
          </h3>
            ...
            {tabs.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`flex cursor-pointer items-center gap-2 rounded-full px-5 py-2.5 text-sm font-bold transition-all duration-200 ease-out ${
                    isActive
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <p
                          className={`truncate font-heading text-sm font-bold transition-colors duration-200 ${
