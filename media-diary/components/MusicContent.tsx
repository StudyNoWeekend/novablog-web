...
import { ExternalLink, Headphones, ListMusic, Music2 } from "lucide-react";
...
function SongSkeleton() {
  return (
    <li className="flex animate-pulse items-center gap-4 rounded-xl bg-surface p-4 shadow-card">
...
function PlaylistSkeleton() {
  return (
    <div className="animate-pulse rounded-xl border border-border bg-surface p-4 shadow-card">
      <div className="aspect-[3/2] w-full rounded-lg bg-text-subtle/30" />
...
function PlaylistCard({ playlist }: { playlist: Playlist }) {
  return (
    <a
      ...
      className="group block rounded-xl border border-border bg-surface p-4 shadow-card transition-all duration-200 ease-out hover:-translate-y-1 hover:shadow-card-hover"
    >
      {/* Cover */}
      <div className="relative aspect-[3/2] w-full overflow-hidden rounded-lg bg-background-soft">
        {playlist.cover_url ? (
          <Image
...
/**
 * 音乐页内容：点击歌曲即加入底部播放条队列（B 站外链引擎），
 * 播放控制、模式切换与进度拖拽由全局 FloatingPlayer 承担。
 */
