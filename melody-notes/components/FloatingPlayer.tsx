...
export function FloatingPlayer() {
  ...
  return (
    <aside
      className="fixed bottom-4 right-4 z-50 w-[calc(100vw-2rem)] max-w-[340px] overflow-hidden rounded-xl border border-border bg-surface shadow-glow"
      role="group"
      aria-label="音乐播放器"
    >
      {/* Header：歌曲信息 + 最小化/关闭 */}
        ...
        {player.urlError ? (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-surface-elevated px-4 text-center">
            ...
            <button
              ...
              className="flex cursor-pointer items-center gap-1.5 rounded-lg border border-border px-3 py-1.5 text-xs text-text-secondary transition-colors duration-200 hover:border-accent hover:text-accent"
            >
