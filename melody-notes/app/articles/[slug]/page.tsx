    ...
    <div className="flex flex-1 flex-col">
      ...
      {article.cover_image ? (
        <header className="mx-auto w-full max-w-5xl px-4 py-6 sm:px-6 lg:px-8">
          <div className="relative aspect-[21/9] overflow-hidden rounded-xl border border-border shadow-card">
            <Image
            ...
            <div className="absolute inset-x-0 bottom-0 p-6 md:p-8">
              {article.category_name && (
                <span className="mb-3 inline-block rounded-full bg-accent-strong px-3 py-1 text-xs font-medium text-on-accent">
                  {article.category_name}
                </span>
              )}
        ...
        <div className="flex flex-wrap items-center gap-4 text-sm text-text-muted">
          {formattedDate && (
            <span className="flex items-center gap-1.5">
              <CalendarDays className="h-4 w-4 text-text-subtle" strokeWidth={1.5} />
              {formattedDate}
            </span>
          )}
          <span className="flex items-center gap-1.5">
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            {article.tag_names.map((tag) => (
              <Link
                ...
                className="min-h-8 cursor-pointer rounded-full border border-accent/25 bg-accent-subtle px-3 py-0.5 text-xs text-accent transition-colors duration-200 hover:border-accent hover:bg-accent hover:text-on-accent"
              >
                #{tag}
              </Link>
            ))}
