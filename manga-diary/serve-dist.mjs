 ...
 * 并把 /api/v1 代理到 mock API（默认 localhost:8333），模拟同域部署。
 * 用法：node serve-dist.mjs  （服务 dist/，默认端口 3377）
 */
...
const PORT = Number(process.env.SERVE_PORT || 3377);
const API_TARGET_HOST = "localhost";
const API_TARGET_PORT = Number(process.env.MOCK_PORT || 8333);
...
const FALLBACKS = [{ prefix: "/articles/", file: "article.html" }];
