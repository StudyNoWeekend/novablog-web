  ...
  return (
    <>
      ...
      <aside
        role="group"
        aria-label="音乐播放器"
        className="fixed inset-x-0 bottom-0 z-50 border-t-2 border-ink bg-surface shadow-[0_-8px_32px_rgba(42,37,28,0.16)] animate-[player-slide-up_0.3s_ease-out] motion-reduce:animate-none"
      >
        <div className="mx-auto flex h-24 w-full max-w-3xl flex-col justify-center gap-1 px-4">
          ...
          {player.urlError ? (
            <div className="flex h-5 items-center justify-center gap-2 text-xs text-text-muted">
              ...
              <button
                ...
                className={`flex cursor-pointer items-center gap-1 rounded-full border border-border px-2 py-0.5 text-xs text-text-secondary transition-colors duration-200 hover:border-accent hover:text-accent-hover ${focusRing}`}
              >
          ...
          <div className="flex h-14 items-center gap-2 sm:gap-3">
            <div className="relative h-10 w-10 shrink-0 overflow-hidden rounded-full border-2 border-accent bg-background-soft">
              {song.cover_url ? (
                <Image
                  src={song.cover_url}
                  alt={song.title}
            ...
            <button
              ...
              className={`flex h-10 shrink-0 cursor-pointer items-center gap-1.5 rounded-full px-2 text-accent-hover transition-colors duration-200 hover:bg-accent-subtle sm:px-2.5 ${focusRing}`}
            >
            ...
            <button
              ...
              aria-label="上一首"
              className={`flex h-10 w-10 shrink-0 cursor-pointer items-center justify-center rounded-full text-text-secondary transition-colors duration-200 hover:bg-background-soft hover:text-accent-hover disabled:cursor-not-allowed disabled:opacity-50 ${focusRing}`}
            >
            ...
            <button
              ...
              className={`flex h-11 w-11 shrink-0 cursor-pointer items-center justify-center rounded-full bg-ink text-accent shadow-lg transition-transform duration-200 hover:scale-105 ${focusRing}`}
            >
              {player.isPaused ? (
