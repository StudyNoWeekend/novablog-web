...
function SongSkeleton() {
  return (
    <li className="flex animate-pulse items-center gap-4 rounded-radius-sm bg-background-soft p-4">
      <div className="h-10 w-6 rounded bg-border-strong/50" />
      <div className="h-12 w-12 rounded-full bg-border-strong/50" />
      <div className="flex-1">
        <div className="h-3.5 w-40 rounded bg-border-strong/50" />
...
function PlaylistSkeleton() {
  return (
    <div className="animate-pulse rounded-radius-md border border-border bg-surface p-4 shadow-card">
      <div className="aspect-[3/2] w-full rounded-radius-sm bg-border-strong/50" />
      <div className="mt-4 h-4 w-3/4 rounded bg-border-strong/50" />
...
function PlatformBadge({ platform }: { platform: string }) {
  return (
    <span className="shrink-0 rounded-full bg-accent px-2.5 py-0.5 text-[11px] font-medium text-ink">
      {platform}
    </span>
  );
}
...
function PlaylistCard({ playlist }: { playlist: Playlist }) {
  return (
    <a
      ...
      className="group block rounded-radius-md border border-border bg-surface p-4 shadow-card transition-all duration-200 ease-out hover:-translate-y-1 hover:shadow-card-hover"
    >
      {/* Cover */}
      <div className="relative aspect-[3/2] w-full overflow-hidden rounded-radius-sm border border-border bg-background-soft">
        {playlist.cover_url ? (
          <Image
...
/**
 * 音乐分享：歌曲列表（点击经底部播放条播放，B 站外链 iframe）+ 第三方歌单。
 */
              ...
              return (
                <button
                  ...
                  className={`flex cursor-pointer items-center gap-2 rounded-full px-5 py-2.5 text-sm font-medium transition-all duration-200 ease-out ${
                    isActive
                      ? "bg-ink text-accent shadow-card"
        ...
        {activeTab === "songs" && (
          <>
            {songsLoading ? (
              ...
              <ul className="divide-y divide-border overflow-hidden rounded-radius-md border-2 border-ink bg-surface shadow-card">
                {songs.map((song, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex w-8 shrink-0 justify-center">
                        {isActive ? (
                          ...
                          <span
                            ...
                            aria-label="当前歌曲"
                          >
                            <span className="h-[45%] w-[3px] rounded-full bg-accent-hover" />
                            <span className="h-full w-[3px] rounded-full bg-accent-hover" />
                            <span className="h-[70%] w-[3px] rounded-full bg-accent-hover" />
                          </span>
                        ) : (
                          <span className="font-hand text-base tabular-nums text-text-subtle">
                      ...
                      {/* Cover */}
                      <div className="relative h-12 w-12 shrink-0 overflow-hidden rounded-full border-2 border-border bg-background-soft">
                        {song.cover_url ? (
                          <Image
                            src={song.cover_url}
                            alt={song.title}
            ...
            {songs.length > 0 && (
              <p className="mt-4 text-center font-hand text-lg text-text-muted">
                点一首歌，让画画的时间更有味道 ♪
              </p>
            )}
          </>
