    ...
    <div className="flex flex-1 flex-col bg-background">
      ...
      {article.cover_image ? (
          ...
          <div className="relative aspect-[21/9] overflow-hidden rounded-radius-lg shadow-card">
            ...
            <div className="absolute inset-x-0 bottom-0 p-6 md:p-8">
              {article.category_name && (
                <span className="mb-3 inline-block rounded-full bg-accent px-3 py-1 text-xs font-medium text-background">
                  {article.category_name}
                </span>
              )}
        ...
        <header className="mx-auto w-full max-w-3xl px-4 pt-4 sm:px-6 lg:px-8">
          ...
          <h1 className="font-[var(--font-playfair)] text-3xl font-bold leading-tight text-text-primary md:text-4xl">
            {article.title}
          </h1>
        </header>
      )}
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            {article.tag_names.map((tag) => (
              <Link
                ...
                className="min-h-8 cursor-pointer rounded-full border border-accent/25 bg-accent-subtle px-3 py-0.5 text-xs text-accent transition-colors duration-200 hover:border-accent hover:bg-accent hover:text-background"
              >
                #{tag}
              </Link>
            ))}
