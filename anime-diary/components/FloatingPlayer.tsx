  ...
  return (
    <>
      ...
      <aside
        role="group"
        aria-label="音乐播放器"
        className="fixed inset-x-0 bottom-0 z-50 animate-[player-slide-up_0.3s_ease-out] border-t border-accent/20 bg-white/95 shadow-[0_-8px_32px_rgba(60,53,84,0.14)] backdrop-blur-md motion-reduce:animate-none"
      >
        <div className="mx-auto flex h-24 w-full max-w-3xl flex-col justify-center gap-1 px-4">
          ...
          <div className="flex h-14 items-center gap-2 sm:gap-3">
            <div className="relative h-10 w-10 shrink-0 overflow-hidden rounded-full border border-accent/30 bg-background-soft">
              {song.cover_url ? (
                <Image
                  src={song.cover_url}
                  alt={song.title}
            ...
            <button
              ...
              className={`flex h-11 w-11 shrink-0 cursor-pointer items-center justify-center rounded-full bg-accent text-white shadow-lg transition-transform duration-200 hover:scale-105 ${focusRing}`}
            >
              {player.isPaused ? (
