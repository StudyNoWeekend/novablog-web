...
import { ArticleCard, categoryBadgeStyle } from "@/components/ArticleCard";
...
export default async function ArticleDetailPage({ params }: PageProps) {
  ...
  if (!config.article_enabled) {
    return <ModuleDisabled moduleLabel="动态" />;
  }
    ...
    <div className="flex flex-1 flex-col bg-background">
      ...
      {article.cover_image ? (
          ...
          <div className="relative aspect-[21/9] overflow-hidden rounded-radius-lg shadow-card">
            ...
            <div className="absolute inset-x-0 bottom-0 p-6 md:p-8">
              {article.category_name && (
                <span
                  className={`mb-3 inline-block rounded-full px-3 py-1 text-xs font-medium ${categoryBadgeStyle(article.category_id)}`}
                >
                  {article.category_name}
                </span>
              )}
              <h1 className="max-w-3xl font-heading text-2xl font-normal leading-tight text-white md:text-4xl">
                {article.title}
              </h1>
            </div>
          </div>
        ...
        <header className="mx-auto w-full max-w-3xl px-4 pt-4 sm:px-6 lg:px-8">
          ...
          <h1 className="font-heading text-3xl leading-tight text-text-primary md:text-4xl">
            {article.title}
          </h1>
        </header>
      )}
      ...
      <div className="mx-auto w-full max-w-3xl flex-1 px-4 py-8 sm:px-6 lg:px-8 md:py-10">
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            {article.tag_names.map((tag) => (
              <Link
                ...
                className="min-h-8 cursor-pointer rounded-full border border-accent/25 bg-accent-subtle px-3 py-0.5 text-xs text-accent-hover transition-colors duration-200 hover:border-accent hover:bg-accent hover:text-white"
              >
                #{tag}
              </Link>
            ))}
        ...
        {/* Summary quote */}
        {article.summary && (
          <blockquote className="mt-8 rounded-radius-md border-l-4 border-accent bg-accent-subtle/50 px-5 py-4 text-base italic leading-relaxed text-text-secondary">
            {article.summary}
          </blockquote>
        )}
      ...
      {filteredRelated.length > 0 && (
        <section className="border-t border-border bg-background-soft py-12 md:py-16">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <h2 className="mb-8 font-heading text-2xl text-text-primary">
              推荐阅读
            </h2>
